...
function executeCommand(command, args) {
    return new Promise((resolve, reject) => {
        const proc = (0, child_process_1.spawn)(command, args, { shell: false });
        let stdout = '';
...
function validatePid(pid) { return Number.isInteger(pid) && pid > 0; }
function validatePort(port) { return Number.isInteger(port) && port >= 1 && port <= 65535; }
// LOGIC
async function findAntigravityProcess() {
    ...
    const match = lines.find(line => line.includes(PROCESS_IDENTIFIERS.ANTIGRAVITY) ||
        line.includes(PROCESS_IDENTIFIERS.CSRF_TOKEN));
    if (!match)
        throw new Error("Could not find Antigravity process.");
    const pidMatch = match.trim().match(/^(\d+)/);
    if (!pidMatch)
        throw new Error("Could not parse PID.");
...
async function queryStatus(port, csrfToken) {
    return new Promise((resolve, reject) => {
    ...
    try {
        ...
        for (const port of ports) {
            ...
            try {
                ...
                const res = await queryStatus(port, csrfToken || '');
